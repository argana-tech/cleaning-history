...
typedef enum {
   ...
   BatteryStatusUndefined                    /**< Unable to determin the battery level */
} BatteryStatus;
...
/**
 @brief Enumeration of the Codabar check character settings
 */
typedef enum {
    CodabarCheckCharNoCheckChar,                        /**< Disable the check character */
    CodabarCheckCharValidateNotTransmitted,             /**< Vaildate the check character but do not send it with the barcode data */
    CodabarCheckCharValidateAndTransmit                 /**< Vaildate the check character and send it with the barcode data */
} CodabarCheckChar;
...
typedef enum {
    ...
    Interleaved2of5CheckCharValidateNotTransmitted,         /**< Vaildate the check character but do not send it with the barcode data */
    Interleaved2of5CheckCharValidateAndTransmit             /**< Vaildate the check character and send it with the barcode data */
...
typedef enum {
    ...
    NEC2of5CheckCharValidateNotTransmitted,                 /**< Vaildate the check character but do not send it with the barcode data */
    NEC2of5CheckCharValidateAndTransmit                     /**< Vaildate the check character and send it with the barcode data */
} NEC2of5CheckChar;
...
/**
 ...
 Each method defined in the protocol represents a specific response from SDK actions. Each is optional and only needs to be implemented by delegate objects that are interested in the specific repsonce.
  */
...
/**
 @brief This delegate method is called when the decoder reads string data from a barcode.
...
/**
 ...
 The delegate message passes the decoder driver revision data from the <em>-(void)requestDecoderDriverRevision</em> method. This revision number is for the decoder driver on the device.
...
/**
 ...
 The delegate message passes the symbology that is set as the high priroity. It is requested from the <em>-(void)requestDecoderHighPrioritySymbologyStatus</em> method.

 @param (Symbology)symbology The symbology that is currently set as the high priority.
 */
...
/**
 ...
 The delegate message passes the symbology that is set as the low priroity. It is requested from the <em>-(void)requestDecoderLowPrioritySymbologyStatus</em> method.

 @param (Symbology)symbology The symbology that is currently set as the low priority.
 */
...
/**
 @brief This delegate method is called when the left of centering window location is requested.
...
/**
 @brief This delegate method is called when the right of centering window location is requested.
...
/**
 @brief This delegate method is called when data is returned from unsing the direct pass though method for the decoder.

 The delegate message passes repsonse data from the use of the <em>-(void)decoderPassThrough:(NSData*)data expectingReturnData:(BOOL)returnData</em> method. The data received from this method is directly returned as a NSData.
...
/**
 ...
 When data from a card swipe is read it is sent to delegates using this method. The <em>status</em> paramater will be <em>YES</em> if the checksum received from the MSR matches the expected calaulated checksum. In most cases this can be ingnored because it will always be <em>YES</em> except for the rare occasions of a hardware issue.
...
/**
 @brief This delegate method is called when data is returned from unsing the direct pass though method for the MSR.

 The delegate message passes repsonse data from the use of the <em>-(void)msrPassThrough:(NSData*)data expectingReturnData:(BOOL)returnDataa</em> method. The data received from this method is directly returned as a NSData.
...
/**
 @brief This delegate method is called when data is returned from unsing the direct read isled part number&Configure number&Serial Number.
...
/**
 ...
 This should be inclueded in all projects that intend to use the Honeywell Captuvo sled. All methods in this SDK should be assumed to not be thread safe. The ExternalAccessory.framework
 */
...
/**
 ...
 When using this SDK, this is the proper way to initilize and get a referance to a Captuvo object.
...
/**
 @brief This method is used to get the shared instance of the Captuvo object and place the SDK into debug mode with verbos logging.

 This will behave the same as the <em>+(Captuvo*)sharedCaptuvoDevice</em> method with the addtion of placing the SDK into debug mode. This only needs to be called once to enter debug mode. Debug mode can be turned off by using the <em>-(void)enableCaptuvoDebug:(BOOL)enable</em> method.
...
/**
 @brief This method is used to registere an object as a delegate for SDK events.

 To receive data back from the SDK delegates most be registered with it. Delegate objects are unable to be added twice and objects that are deferenced are automatically removed.

 @param (id<CaptuvoEventsProtocol>)delegate The object to be registere as a active delegate.
 */
-(void)addCaptuvoDelegate:(id<CaptuvoEventsProtocol>)delegate;//

/**
 @brief This method is used to unregistere an object as a delegate for SDK events.

 This method will unregister an object as a delegate for the SDK. If the object is not a vaild deleage no changes will take place.

 @param (id<CaptuvoEventsProtocol>)delegate The object to be unregistere as a active delegate.
 */
...
/**
 ...
 @return A NSString that is the devices serial number.
 */
...
/**
 ...
 @return A NSString that is the devices firmware revision.
 */
...
/**
 ...
 @return A NSString that is the devices hardware revision.
 */
-(NSString*)getCaptuvoHardwareRevision;//

/**
 @brief This method returns the manufacturer informtaion of the device.

 This method will return the manufacturer informtaion of the device.

 @return A NSString that is the devices manufacturer informtaion.
 */
...
/**
 ...
 This must be called before any other decoder methods. Untill this method is called the decoder hardware is not initialize. Use <em>-(void)stopDecoderHarware</em> to shut down the decoder hardware
...
/**
 ...
 It will retry to turn on the decoder hardware every 250ms untill succeed or timeout.
...
/**
 ...
 Once this method is called the decoder hardware is unactivated. No scanning methods can be called untill <em>-(void)startDecoderHardware</em> is called again to re-initialize the decoder hardware
 */
...
/**
 ...
 This method is used to determine if the decoder is currently running. It can be started and stoped with the methods <em>-(void)startDecoderHardware</em> and <em>-(void)stopDecoderHarware</em> respectivly.
...
/**
 ...
 This method enables the scanner to be controlled by software. Once this command is sent the scanner will search for a barcode until eiather it finds one, timesouts or the <em>-(void)stopDecoderScanning</em> method is called.
...
/**
 ...
 This method can be used to stop the scanner after the <em>-(void)startDecoderScanning</em> was called. If the decoder is not scanning at the time their will be no effect of this method call.
 */
...
/**
 ...
 This method disable the scanner.   The method <em>-(void)enableDecoderScanning</em> should be used to renable scanning.  Calling <em>-(void)disableDecoderScanning</em> does not enable/disable power to the decoder.
 */
...
/**
 ...
 This methods sets whether a audable beep will be heard when the decoder is powered up. The default setting is no beep.
...
/**
 ...
 This methods sets whether a audable click will be heard when the hardware scan button is pressed. The default setting is no click.
...
/**
 ...
 This allows the volumn of the beeper to be set after a successful read.
...
/**
 @brief Set the beeper pitch on a error read.
...
/**
 ...
 This allows the numbers of beeps to be set after a successful read. The number of beeps must be set to between 1 and 9 inclusively. A number outside of this range will be rounded to the nearest vaild number.
...
/**
 ...
 This allows the numbers of beeps to be set after an error. The number of beeps must be set to between 1 and 9 inclusively. A number outside of this range will be rounded to the nearest vaild number.
...
/**
 ...
 This allows a delay in milliseconds to be set between reads. The decoder will not read a second barcode untill after the delay period. The delay can be between 0 and 30,000 milliseconds inclusively. A number outside of this range will be rounded to the nearest vaild number.
...
/**
 ...
 After the decoder successfully scans it will not scan another untill the delay timer timesout. This method requests the duration of this delay in milliseconds. The result of this method is returned with the <em>-(void)requestDecoderGoodReadDelayInMilliSecondsStatus</em> delegate method.
 */
...
/**
 @brief Set the timeout period of a scan initated from software.

 This allows the timeout period of a scan that was initated from software (not the hardware buttom) to have a timeout period set. The timeout can be set from 0 to 300,000 milliseconds inclusively. A number outside of this range will be rounded to the nearest vaild number.
...
/**
 @brief This method is used to request the delay in milliseconds for the scanner to turnoff if unable to find a barcode.

 Once the decoder is fired this timeout in milliseconds is the amount of time it will remain seraching for a barcode untill is turns off. This method requests the duration of this delay in milliseconds. The result of this method is returned with the <em>-(void)decoderSerialTriggerTimeoutInMilliSecondsStatus:(int)milliseconds</em> delegate method.
 */
-(void)requestDecoderSerialTriggerTimeoutInMilliSecondsStatus;

/**
 @brief Set the decoder to optimze for reading mobile phone screens.

 When this mode is enabled the decoder is optimized to read bar codes from mobile phone or other LED displays. However, the speed of scanning printed bar codes may be slightly lower when this mode is enabled.
...
/**
 ...
 The docder is able to perfer one symbology over another. This method requests the status of this setting. The result of this method is returned with the <em>-(void)decoderPreferredSymbologyStatus:(BOOL)isEnabled</em> delegate method.
 */
-(void)requestDecoderPreferredSymbologyStatus;

/**
 @brief Set the preferred symbology back to its defalut settings

 This allows the the decoder to be set back to its defalut settings for preferred symbology.
...
/**
 @brief This method is used to request the delay in milliseconds for the perferred symbology setting.

 If the decoder finds a barcode which is not its perferred symbology it will continue to look for another bar code with a higher priority. This method requests the duration of this delay in milliseconds. The result of this method is returned with the <em>-(void)decoderPreferredSymbologyTimeoutInMilliSecondsStatus:(int)milliseconds</em> delegate method.
 */
...
/**
 ...
 The deocder can enable the centering feature to restrict its field of view to the center of the scan area. This method will turn on this feature.
...
/**
 ...
 The deocder can enable the centering feature to restrict its field of view to the center of the scan area. This method will configure the location of the top of the centering window in relation to the top of the maximum field of view as a precent.

 @param (int)locationAsPercent The precent from top of the maximum field of view to place the top of the centering window.
 @param (BOOL)persist Set to YES to save the setting between power cycles.
 */
-(void)setDecoderTopOfCenteringWindow:(int)locationAsPercent persistSetting:(BOOL)persist;//

/**
 @brief This method is used to request the location of the top of centering window.

 The decoder can be configured to limit its field of view to just the center area. This method will request the location of the top of centering window. The result of this method is returned with the <em>-(void)decoderTopOfCenteringWindowLocation:(int)locationAsPrecent</em> delegate method.
 */
...
/**
 ...
 The deocder can enable the centering feature to restrict its field of view to the center of the scan area. This method will configure the location of the bottom of the centering window in relation to the top of the maximum field of view as a precent.

 @param (int)locationAsPercent The precent from top of the maximum field of view to place the bottom of the centering window.
 @param (BOOL)persist Set to YES to save the setting between power cycles.
 */
...
/**
 ...
 The decoder can be configured to limit its field of view to just the center area. This method will request the location of the bottom of centering window. The result of this method is returned with the <em>-(void)decoderBottomOfCenteringWindowLocation:(int)locationAsPrecent</em> delegate method.
 */
...
/**
 ...
 The deocder can enable the centering feature to restrict its field of view to the center of the scan area. This method will configure the location of the left side of the centering window in relation to the left side of the maximum field of view as a precent.

 @param (int)locationAsPercent The precent from left side of the maximum field of view to place the left side of the centering window.
 @param (BOOL)persist Set to YES to save the setting between power cycles.
 */
...
/**
 ...
 The decoder can be configured to limit its field of view to just the center area. This method will request the location of the left side of centering window. The result of this method is returned with the <em>-(void)decoderLeftOfCenteringWindowLocation:(int)locationAsPrecent</em> delegate method.
 */
...
/**
 ...
 The deocder can enable the centering feature to restrict its field of view to the center of the scan area. This method will configure the location of the right side of the centering window in relation to the left side of the maximum field of view as a precent.

 @param (int)locationAsPercent The precent from left side of the maximum field of view to place the right side of the centering window.
 @param (BOOL)persist Set to YES to save the setting between power cycles.
 */
...
/**
 ...
 The decoder can be configured to limit its field of view to just the center area. This method will request the location of the right side of centering window. The result of this method is returned with the <em>-(void)decoderRightOfCenteringWindowLocation:(int)locationAsPrecent</em> delegate method.
 */
...
/**
 @brief This method enables all avaibale symbologies to be decoded.
...
/**
 @brief This method disable alls symbologies.

 This method is used to disable the decoding of all symbologies. This is nomrally used in the case where an applaction only wants to scan a select few sysmboloies. First this will be used to turn off all of the symbologies and then the symbologies that are required are turned on one by one.
...
/**
 ...
 This method is used to configure the UP-CA symbology. An UPCA object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the EAN-13 symbology. An EAN13 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the UPC-E0 symbology. An UPCE0 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the EAN-8 symbology. An EAN8 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Codabar symbology. A Codabar object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Code 39 symbology. A Code39 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Interleaved 2 of 5 symbology. An Interleaved2of5 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the NEC 2 of 5 symbology. A NEC2of5 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Code 93 symbology. A Code93 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Straight 2 of 5 Industrial symbology. A Straight2of5Industrial object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Matrix 2 of 5 symbology. A Matrix2of5 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Code 11 symbology. A Code11 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Code 128 symbology. A Code128 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the GS1-128 symbology. A GS1_128 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Telepen symbology. A Telepen object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the MSI symbology. A MSI object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the GS1 DataBar Omnidirectional symbology. A GS1DataBarOmnidirectional object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the GS1 DataBar Limited symbology. A GS1DataBarLimited object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the GS1 DataBar Expanded symbology. A GS1DataBarExpanded object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Codablock A symbology. A CodablockA object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Codablock F symbology. A CodablockF object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the PDF417 symbology. A PDF417 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Data Matrix symbology. A DataMatrix object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Aztec symbology. An Aztec object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the Chinese Sensible symbology. A ChineseSensible object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the MicroPDF417 symbology. A MicroPDF417 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the TCIF Linked Code 39 symbology. A TCIFLinkedCode39 object is used hold the configuration options.
...
/**
 ...
 This method is used to configure the QR Code symbology. A QRCode object is used hold the configuration options.
...
/**
 ...
 The data is directly sent without any manipulation of the SDK. The caller is responsable for formatting the message correctly, The caller of this method must indicate if they expect a return message.

 @param (NSData*)data A NSData object that contains the raw data to send to the decoder.
...
/**
 ...
 This must be called before any other MSR methods. Untill this method is called the MSR hardware is not initialize. Use -stopMSRHarware to shut down the MSR hardware.
...
/**
 ...
 It will retry to turn on the MSR hardware every 250ms untill succeed or timeout.
...
/**
 ...
 Once this method is called the MSR hardware is unactivated. No MSR methods can be called untill -startMSRHardware is called again to re-initialize the MSR hardware.
 */
...
/**
 ...
 This method is used to determine if the MSR is currently running. It can be started and stoped with the methods <em>-(void)startMSRHardware</em> and <em>-(void)stopMSRHarware</em> respectivly.
...
/**
 @brief This method requests the firmware version from the MSR hardware.
...
/**
 ...
 When this method is called a request is sent to the MSR hardware for its serial number. This data is returned with the <em>-(void)msrSerialNumber:(NSString*)data validData:(BOOL)status</em> delegate method.
 */
...
/**
 ...
 When this method is called a request is sent to the MSR hardware for its current security level. This data is returned with the <em>-(void)msrCurrentSecurityLevel:(SecurityLevel)securityLevel validData:(BOOL)status</em> delegate method.
 */
-(void)requestMSRSecurityLevel;

/**
 @brief This method requests the track settings from the MSR hardware.

 When this method is called a request is sent to the MSR hardware for the current tacks that the device will read. This data is returned with the <em>-(void)msrCurrentTrackSelection:(TrackSelection)trackSelection validData:(BOOL)status</em> delegate method.
 */
...
/**
 ...
 The data is directly sent without any manipulation of the SDK. The caller is responsable for formatting the message correctly and appling the correct checksum. The caller must also indicate if they expect a return message.

 @param (NSData*)data A NSData object that contains the raw data to send to the MSR.
...
#pragma mark -
#pragma mark Power Managment Methods

/**
 @brief This method is used to get the current charing status.
...
/**
 ...
 When this method is called to request the current barrery voltage of the device. This data is returned with the <em>-(void)pmBatteryVoltage:(float)voltage</em> delegate method. To track battery status it is better to use the <em>-(BatteryStatus)requestBatteryStatus</em> method.
 */
-(void)requestBatteryVoltage;


/**
 @brief This method requests the current charge status of a batery.
